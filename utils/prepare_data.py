...
def load_data(input_file, tokenizer, max_doc_len=75, max_sen_len_bert=60, max_cau_num=3):
    print('load data_file: {}'.format(input_file))
    x_bert, sen_len_bert, doc_len, y, doc_id = [], [], [], [], []
    con_doc_len = []
    emo, cau, con = [], [], []
    ...
    while True:
        ...
        sen_len_bert_tmp = np.zeros(max_doc_len, dtype=np.int32)
        x_bert_tmp = np.zeros((max_doc_len, max_sen_len_bert), dtype=np.int32)
        cau_cnt = 0
        con_cnt = 0
    ...
    y, x_bert, sen_len_bert, doc_len, emo, cau, con, con_doc_len = map(np.array, [y, x_bert, sen_len_bert, doc_len, emo, cau, con, con_doc_len])
    for var in ['y', 'x_bert', 'sen_len_bert', 'doc_len', 'emo', 'cau', 'con']:
        print('{}.shape {}'.format(var, eval(var).shape))
    print('n_cut {}'.format(n_cut))
    print('load data done!\n')
    return doc_id, x_bert, sen_len_bert, doc_len, y, emo, cau, con, con_doc_len
